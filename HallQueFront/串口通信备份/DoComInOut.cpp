...
//			DWORD dwWrited = 0;
			...
			do
			{
				...
				BOOL bWriteStat = WriteFile(pComInit->m_hComWndScreen,pMsg->buf,
					pMsg->length,NULL,&osWrite);
				if(!bWriteStat)
				{
					if(GetLastError()==ERROR_IO_PENDING)
					{	 
						WaitForSingleObject(pComInit->m_hComWndScreen,INFINITE);
					}
				}
				GetOverlappedResult(pComInit->m_hComWndScreen,&osWrite,&dwTrans,TRUE);
				dwAcWrite += dwTrans;
#ifdef _DEBUG
				CString strCount;
...
#ifdef _DEBUG
				CString str;
				str.Format(_T("write com count:%d"),dwAcWrite);
				MyWriteConsole(str);
#endif
			delete pMsg;
		}
