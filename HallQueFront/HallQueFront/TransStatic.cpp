...
//		rc.InflateRect(-2,-2);//让文字距左右，上下1个像素
		...
		{
		case enumLeft:
			::DrawText(hDC,strText,-1,&rc,DT_LEFT|DT_WORDBREAK|DT_EDITCONTROL);
			break;
		case enumCenter:
			::DrawText(hDC,strText,-1,&rc,DT_CENTER|DT_WORDBREAK|DT_EDITCONTROL);
			break;
		case enumRight:
			::DrawText(hDC,strText,-1,&rc,DT_RIGHT|DT_WORDBREAK|DT_EDITCONTROL);
			break;
		}
...
void CTransStatic::SetWindowText(CString strText)
{
	m_strText = strText;
}

void CTransStatic::GetWindowText(CString& strText)
{
	strText = m_strText;
}