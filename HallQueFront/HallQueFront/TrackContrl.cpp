...
BOOL CTrackContrl::DeleteRightBnSelectCtr()
{
	if(m_pRightBnSelect)
	{
		...
		for(int i=0;i<m_selTrackerAll.GetSize();i++)
		{
			...
			{
//				m_pRightBnSelect->DestroyWindow();
...
void CTrackContrl::SetAllCtrlHide()
{
	for(int i=0;i<m_selTrackerAll.GetSize();i++)
	{
		m_selTrackerAll[i]->ShowWindow(SW_HIDE);
	}
}

void CTrackContrl::SetAllCtrlShow()
{
	for(int i=0;i<m_selTrackerAll.GetSize();i++)
	{
		m_selTrackerAll[i]->ShowWindow(SW_SHOWNORMAL);
	}
}

void CTrackContrl::SetAllCtrlLinkPageIDNULL(UINT nPageID)
{
	for(int i=0;i<m_selTrackerAll.GetCount();i++)
	{
		if(m_selTrackerAll[i]->GetWindowType()==enmButton)
		{
			if(m_selTrackerAll[i]->m_pTransButton->GetAttachPageID()==nPageID)
			{
				m_selTrackerAll[i]->m_pTransButton->AttachToPage(-1);
			}
		}
	}
}