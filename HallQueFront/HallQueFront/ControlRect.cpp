...
CControlRect::CControlRect(void) : m_nPageID(0)
{
	Initialize();
}
...
CControlRect::~CControlRect(void)
{
	if(m_pTransButton)
	{
		m_pTransButton->DestroyWindow();
		delete m_pTransButton;
	}
	if(m_pTransStatic)
	{
		m_pTransStatic->DestroyWindow();
		delete m_pTransStatic;
	}
}

CControlRect::CControlRect(CRect& rect) : m_nPageID(0)
{
						  ...
						  UINT nID,CCreateContext* pContext,CString loadPicPath)
{
	...
	if(windtype == enmStatic)
	{
		...
		pTransStatic->SetWindowText(lpszWindowName);
...
void CControlRect::PostNcDestroy()
{
	// TODO: 在此添加专用代码和/或调用基类
