...
BOOL SLZPrinter::LoadPrintRectFormFile()
{
	...
	if(file.Open(path,CFile::modeRead))
	{
		ULONGLONG count=file.GetLength()/sizeof(POINT);
		for(ULONGLONG i=0;i<count;i++)
		{
			if(file.Read(&pt[i],sizeof(POINT))>0)
			{
				continue;
			}
		}
		m_printRect.left = pt[0].x;m_printRect.top = pt[0].y;
		m_printRect.right = pt[1].x;m_printRect.bottom = pt[1].y;
		return TRUE;
	}
	else
	{
...
void SLZPrinter::PrintInfo(const SLZData& data)
{
#ifdef _DEBUG
...
//	AfxMessageBox(str);
	...
	double timedel = time.GetDuration();
	CString strTime;
	strTime.Format(_T("打印时间:%lf"),timedel);
#endif
}
...
void SLZPrinter::SetPrintStyle(CDC* pDC,const WINDOWCTRINFO& info,const SLZData& data)
{
...
//		pt.x = rect.left;pt.y = rect.top;
		...
		if(!content.IsEmpty())
		{
//			int col = FlushCStringData(pDC,content,rect,info);
//			pDC->TextOut(pt.x,pt.y,content,content.GetLength());
//			rect.bottom*=col;
			::DrawText(pDC->GetSafeHdc(),content,-1,&rect,DT_LEFT|DT_WORDBREAK|DT_EDITCONTROL);
		}
	}
...
{
	return LoadPrintInofFromFile();
}

int SLZPrinter::FlushCStringData(CDC* pDC,const CString& content,const CRect& rect,const WINDOWCTRINFO& info)
{
	//如果文字大于一行长度、进行格式化
//	CString strValue;
	int col = 1;
	...
	int width=tm.tmAveCharWidth; //character width
	if(rect.left + len*width > rect.right)
	{
		//一行最多显示多少个字符
		int maxShow = (rect.right-rect.left)/width;
//		if(maxShow==0){strValue=content;return strValue;}
		//文字显示多少行
		col = len/maxShow;
		if(len%maxShow>0)col++;
// 		int startPos = 0;
// 		for(int i=0;i<col;i++)
// 		{
// 			CString strTemp = content.Mid(startPos,maxShow);
// 			strValue+=strTemp;
// 			if(i!=col-1)
// 				strValue+=_T("\n");
// 			startPos+=maxShow;
