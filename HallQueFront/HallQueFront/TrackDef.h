...
protected:
	void Clean()
	{
		memset(address,0,addNum);
		memset(staticPicPath,0,MAX_PATH);
		memset(ctrTextContent,0,textNum);
		memset(forShowQueNumHead,0,addNum);
		memset(bkPicPath,0,MAX_PATH);
		nLinkPageID = -1;
		IsForImage = FALSE;
		IsForPrint = FALSE;
		IsForImage = FALSE;
...
public:
	...
	WCHAR bkPicPath[MAX_PATH];// 背景图片路径
	UINT nPageID;//属于哪个界面
	int nLinkPageID;//链接界面
